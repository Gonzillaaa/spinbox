...
EOF
  ...
  sed -i '' '$ s/,$//' .devcontainer/devcontainer.json 2>/dev/null || sed -i '$ s/,$//' .devcontainer/devcontainer.json
...
# Create backend setup files
function create_backend_setup_files() {
  print_status "Creating backend setup files..."
...
EOF
  ...
  chmod +x backend/setup_local_env.sh
  
  # Create setup.py for backend
...
EOF
  ...
  # Create local virtual environment and initialize files
...
EOF
  ...
  print_status "Backend setup files created."
}
...
function main() {
  ...
  if [ "$USE_BACKEND" = true ]; then
    create_backend_setup_files  # Adding this function first
