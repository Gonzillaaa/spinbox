...
function create_minimal_python_project() {
    ...
    if [[ "${WITH_EXAMPLES:-false}" == "true" ]]; then
        generate_minimal_python_working_examples "$project_dir"
    fi
...
# Generate working examples for minimal Python
function generate_minimal_python_working_examples() {
    local project_dir="$1"
    
    print_info "Adding Python working examples..."
    
    # Create example files directly (since there's no templates directory for minimal)
    cat > "$project_dir/src/example_cli.py" << EOF
#!/usr/bin/env python3
"""
...
EOF
    ...
    cat > "$project_dir/src/example_config.py" << EOF
"""
...
class Config:
    ...
    def __init__(self):
        ...
        self.app_name = self.get_env('APP_NAME', '$PROJECT_NAME')
        self.version = self.get_env('VERSION', '0.1.0')
...
EOF
    ...
    print_info "Python working examples added successfully"
    print_info "Examples available:"
    echo "  • src/example_cli.py - Command line interface example"
    echo "  • src/example_config.py - Configuration management example"
    echo "  • .env.example - Environment variables template"
    echo "  • Run with: python -m src.example_cli --greet"
}
...
export -f generate_minimal_python_working_examples