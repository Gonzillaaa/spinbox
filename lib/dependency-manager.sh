...
function manage_component_dependencies() {
    ...
    if [[ -f "$project_dir/requirements.txt" ]] || [[ "$component" == "fastapi" ]] || [[ "$component" == "postgresql" ]] || [[ "$component" == "redis" ]] || [[ "$component" == "chroma" ]] || [[ "$component" == "mongodb" ]]; then
        add_python_dependencies "$project_dir" "$component"
    fi
    
    # Handle Node.js dependencies
    if [[ -f "$project_dir/package.json" ]] || [[ "$component" == "nextjs" ]] || [[ "$component" == "express" ]]; then
